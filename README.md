...
- SSO: `SSO_POSTGRES_*`, `SSO_HTTP_PORT`, `SSO_GRPC_*`, `SSO_REQUIRE_EMAIL_VERIFICATION`, OAuth/SMTP/JWT параметры.
...
3. Для локальной frontend-разработки поднимите dev-профиль:

```bash
make frontend-up
```

Для полного VPS-сценария с production-фронтендом, HAProxy и выпуском Let's Encrypt сертификата используйте:
...
- Перед `make frontend-prod-cert` или `make frontend-vps-up` укажите в корневом `.env`:
  ...
  - `ALLOWED_REDIRECT_URLS` и `ALLOWED_CORS_ORIGINS` с `https://DOMAIN`
- Основные команды:
  - `make frontend-prod-up` — поднять production-фронтенд и `haproxy` без выпуска сертификата.
  - `make frontend-prod-cert` — выпустить или переиспользовать сертификат и перезапустить `haproxy`.
  - `make frontend-prod-renew` — продлить сертификаты и перезапустить `haproxy`.
  - `make frontend-vps-up` — полный VPS-сценарий: сборка backend + frontend-prod, запуск ingress и выпуск сертификата.
- Базовый flow деплоя:
...
## Тестовый режим почты

- Для локального тестирования без подтверждения почты по SMTP установите в корневом `.env` `SSO_REQUIRE_EMAIL_VERIFICATION=false`.
- В этом режиме новые пользователи создаются уже с `email_confirmed=true`, а логин не блокируется ожиданием письма.
- Этот режим подходит для разработки и QA. Сценарии, которые реально отправляют письма, например password reset, по-прежнему требуют корректный SMTP-конфиг.
